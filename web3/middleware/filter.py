...
async def async_get_logs_multipart(
    ...
    """Used to break up requests to async ``eth_getLogs``
    ...
    for from_block, to_block in _block_ranges:
        ...
        yield await w3.eth.get_logs(
            cast(FilterParams, drop_items_with_none_value(params))
        )
...
class AsyncRequestLogs:
    ...
    @property
    async def to_block(self) -> Awaitable[BlockNumber]:
        if self._to_block is None:
    ...
    def middleware(method: RPCEndpoint, params: Any) -> RPCResponse:
        if method in NEW_FILTER_METHODS:
            ...
            if method == RPC.eth_getFilterChanges:
                ...
                # type ignored b/c logic prevents RequestBlocks which doesn't
                # implement get_logs
...
async def async_local_filter_middleware(
    ...
    async def middleware(method: RPCEndpoint, params: Any) -> RPCResponse:
        if method in NEW_FILTER_METHODS:
            filter_id = next(filter_id_counter)
            _filter: Union[AsyncRequestLogs, RequestBlocks]
            if method == RPC.eth_newFilter:
            ...
            if method == RPC.eth_getFilterChanges:
                ...
                # type ignored b/c logic prevents RequestBlocks
                # which doesn't implement get_logs
